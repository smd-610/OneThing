...
首次 `tauri dev` 需编译约 431 个 Rust crate，后续重编译约 30 秒。SQLite 数据库 `todo.db` 在首次启动时自动创建于系统应用数据目录（Windows: `AppData\Roaming\com.onething.app\`）。
...
- **`ui-store.ts`** — `viewMode`（day/week/month/year/settings）、`selectedDate`（YYYY-MM-DD 字符串）、`theme`（light/dark）。主题通过 localStorage 持久化（key: `onething-theme`），切换时在 `<html>` 上添加/移除 `.dark` class。
...
- 三张表：`todos`（待办）、`settings`（配置键值对）、`sent_reminders`（已发送提醒去重）
- `reorderTodos` 使用 BEGIN/COMMIT/ROLLBACK 事务包裹批量 UPDATE
- 所有查询使用 `$1, $2, ...` 参数化占位符，禁止字符串拼接
- ID 由前端通过 `crypto.randomUUID()` 生成
- `dueDate` 格式为 `YYYY-MM-DDTHH:mm`（含时间）或 `YYYY-MM-DD`（纯日期），按日期过滤时使用 `substr(due_date, 1, 10)`

### 邮件提醒（`src/lib/reminder-scheduler.ts`）

- 每 60 秒检查一次未完成且有具体时间的待办
- 到期前 1 天（1440~1380 分钟窗口）和 1 小时（60~30 分钟窗口）发送邮件
- 通过 Tauri 自定义命令 `send_email`（Rust 侧使用 `lettre` crate 发送 SMTP）
- `sent_reminders` 表防止重复发送
- 设置页（SettingsView）配置 SMTP 和收件邮箱，开关切换即时持久化

### UI 组件

- **shadcn/ui 基础组件** 在 `components/ui/` — 手动编写，非 CLI 生成。使用 `cn()` 工具函数（clsx + tailwind-merge）。
- **共享组件** 在 `components/shared/` — TodoItem（@dnd-kit 拖拽）、TodoForm（Dialog 弹窗，含日期+时间输入）、TodoList（DndContext 包裹）、PriorityBadge、CompletionBar、EmptyState。
- **视图组件** 在 `components/views/` — DayView、WeekView、MonthView、YearView、SettingsView。
...
`dayjs` 使用 `zh-cn` locale（在 `main.tsx` 中设置）。待办日期以 `YYYY-MM-DDTHH:mm` 或 `YYYY-MM-DD` 字符串存储。周起始日为周一（ISO 标准）。`relativeTime` 插件用于到期日相对时间显示。
...
- **Rust 后端：** 仅注册插件（sql、opener）和一个自定义命令 `send_email`（SMTP 发送），其余业务逻辑在 TypeScript 层。
- **SQLite 权限：** 当前授权 `sql:allow-execute`、`sql:allow-select`、`sql:allow-load`、`sql:allow-close`。若需新增 SQL 插件功能，须同步更新 `src-tauri/capabilities/default.json`。

## Cargo.toml 备注

- `tauri` 使用 `default-features = false` 以排除 `compression` 特性（该特性引入 `brotli` 8.x，与部分 Rust 工具链不兼容）。
- 仅启用 `wry` 和 `common-controls-v6` 特性。
- `lettre` 用于 SMTP 邮件发送，启用了 `tokio1-native-tls` 特性。
